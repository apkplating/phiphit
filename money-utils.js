...
//
//  ⚠️ ประวัติการแก้ไข 2026-09-02:
//  เช้าวันนั้นเคยแก้ผิดไปช่วงสั้นๆ โดยเข้าใจผิดว่า grand/net สลับความหมายกันอยู่ (จากการอ่านตัวอย่างใบแรก
//  คลาดเคลื่อน ไม่มีรายการสินค้าให้ตรวจทานไขว้ได้ชัด) ภายหลังตรวจสอบกับใบจริงที่มีรายการสินค้าหลายบรรทัด
//  (ผลรวมรายการ = GRAND TOTAL พอดี) ยืนยันว่าโค้ดต้นฉบับ (ก่อนแก้ช่วงนั้น) ถูกต้องอยู่แล้ว: grandTotal คือ
//  ยอดก่อน VAT (เท่ากับพารามิเตอร์ที่ส่งเข้ามาตรงๆ ไม่บวกอะไรเพิ่ม) ส่วน netTotal คือยอดรวม VAT แล้ว (นำไป
//  แสดงเป็น "รวมเงินสุทธิ NET VALUE" ในเอกสารจริง ซึ่งเป็นตัวเลขที่มากกว่า) — ไฟล์นี้กลับมาเป็นสูตรเดิมแล้ว
//  (เหมือนกับก่อนช่วงบ่ายวันที่ 2026-09-02) ถ้าเคยอัปโหลดเวอร์ชันที่แก้ผิดไปช่วงนั้น INV ที่ออกในช่วงนั้น
//  (ระหว่างอัปโหลดเวอร์ชันผิด จนถึงอัปโหลดไฟล์นี้กลับ) จะมี grandTotal/netTotal สลับกันผิดจริง ต้องตรวจสอบ
//  และแก้ย้อนหลังเฉพาะ INV ที่ออกในช่วงนั้นด้วย recalcInvoiceTotal() ใน job_master.html
// ══════════════════════════════════════════════════════════════════════════
...
// grandTotal: ยอดก่อน VAT (บาท) — เท่ากับ r.grand ตรงๆ (ผลรวมรายการสินค้า เช่น "จำนวนเงินรวม GRAND TOTAL"
// ในเอกสารจริง), rate: อัตราภาษี ค่าเริ่มต้น 0.07 (VAT 7%)
// คืนค่า { grand, vat, net } เป็นบาททศนิยม 2 ตำแหน่งทั้งหมด — net = grand+vat (ยอดรวม VAT แล้ว ตัวใหญ่กว่า
// เสมอ ตรงกับ "รวมเงินสุทธิ NET VALUE" ในเอกสารจริง) พร้อมใช้เก็บ/แสดงผลได้เลย
